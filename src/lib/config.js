...
class ConfigManager {
  constructor(isWatcher = false) {
    ...
    this.fileManager = new Filemage(Path.xunluPath)
  ...
  syncConfigurationKeys() {
    for (const configName of this.defaultConfigList) {
      try {
        const defaultConfig = this.loadConfigData(configName, this.defaultConfigPath)
        ...
        console.warn(`[${PLUGIN_NAME}] 同步配置 ${configName} 失败:`, error.message)
      }
    }
  }
  ...
  loadConfigData(configName, basePath) {
    ...
    try {
      ...
      console.warn(`[${PLUGIN_NAME}] 加载配置文件失败: ${filePath}`, error.message)
      return null
    }
  }
  ...
  getConfigFilePath(configName, basePath) {
    const path = configName.includes(".") ? configName.replace(".", "/") : configName
    return `${Path.xunluPath}${basePath}/${path}.config.yaml`
  }
  ...
  getConfig(configName = "", configType = "user") {
    const basePath = configType === "user" ? this.userConfigPath : this.defaultConfigPath
