...
function write(e) {
  ...
  var a =
    typeof e === "string"
      ? (function (e) {
          n = encodeURIComponent(e).replace(/%([0-9A-F]{2})/g, function (_e, r) {
...
function sum(e, t) {
  ...
  if (t == "hex") {
    i = ""
    for (f = 0; f < 8; f++) i += String(this.reg[f].toString(16)).padStart(8, "0")
  } else {
    i = new Array(32)
    for (f = 0; f < 8; f++) {
      var c = this.reg[f]
...
function generate_rc4_bb_str(url_search_params, user_agent, window_env_str, suffix = "cus", Arguments = [0, 1, 14]) {
  ...
  b[71] = 0
  b[72] =
    b[18] ^ b[20] ^ b[26] ^ b[30] ^ b[38] ^ b[40] ^ b[42] ^ b[21] ^ b[27] ^ b[31] ^ b[35] ^ b[39] ^ b[41] ^ b[43] ^
    b[22] ^ b[28] ^ b[32] ^ b[36] ^ b[23] ^ b[29] ^ b[33] ^ b[37] ^ b[44] ^ b[45] ^ b[46] ^ b[47] ^ b[48] ^ b[49] ^
    b[50] ^ b[24] ^ b[25] ^ b[52] ^ b[53] ^ b[54] ^ b[55] ^ b[57] ^ b[58] ^ b[59] ^ b[60] ^ b[65] ^ b[66] ^ b[70] ^ b[71]
  let bb = [
...
function generate_a_bogus(url_search_params, user_agent, windowEnv = "1536|747|1536|834|0|30|0|0|1536|834|1536|864|1525|747|24|24|Win32") {
  let result_str = generate_random_str() + generate_rc4_bb_str(url_search_params, user_agent, windowEnv)
