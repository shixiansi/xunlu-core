...
export function register(bot) {
  ...
  bot.registerCommand(["^(.+)说"], async ctx => {
    ...
    if (model) {
      let audio = await hobbyist.getModelAudio(model, strarr[1].replace(/\（.*?\）/g, ""))
      console.log(audio)
      if (audio?.msg && !audio.audio_url) {
        return await ctx.reply(audio?.msg)
      }
      let filePath = await downAudioFiles(audio.audio_url, `${model}-${strarr[1].slice(0, 10)}`)
          ...
          ...new Set(
            Object.keys(characterAudioList).map(item => {
              if (item.includes(catelist[index])) {
                return dealCharacterName(item).replace(`${catelist[index]}`, "")
              }
            }),
          ),
        ]
        return await rep.reply(await rep.renderImg("tts", { ttsList: ttslist }))
      } else {
