...
const map = {
    0x01: function () {
        return new writer_1.default()
            ...
            .writeBytes(constants_1.BUF4) //ip
            .writeU16(0);
    },
    0x08: function () {
        return new writer_1.default()
    ...
    0x16: function () {
        const info = this.apk;
        return new writer_1.default()
            .writeU32(info.ssover)
            .writeU32(info.appid)
            .writeU32(info.qr_appid || info.subid)
            .writeBytes(this.device.guid)
            .writeTlv(info.id.slice(0, 32))
            .writeTlv(info.ver)
            .writeTlv(info.sign.length ? info.sign : info.id.slice(0, 32));
    },
    0x18: function () {
        return new writer_1.default()
            .writeU16(1) // ping ver
    ...
    0x1d: function () {
        return new writer_1.default().writeU8(1).writeU32(this.apk.bitmap).writeU32(0).writeU8(0).writeU32(0);
    },
    0x1f: function () {
        return new writer_1.default()
            .writeU8(0) // isRoot
            .writeTlv(this.device.os_type) // OS type
            .writeTlv(this.device.version.release) // OS version
            .writeU16(2) // Network Type
            .writeTlv(this.device.sim) // simOperatorName
            .writeTlv(constants_1.BUF0)
            .writeTlv(this.device.apn); // APN
    },
    0x33: function () {
    ...
    0x106: function (md5pass, nt = false) {
        if (!md5pass?.length)
            return new writer_1.default().writeBytes(this.sig.a1);
        const body = new writer_1.default()
            .writeU16(4) // tgtgt ver
            .writeBytes(crypto.randomBytes(4))
            .writeU32(nt ? 0 : this.apk.ssover) // sso ver
            .writeU32(this.apk.appid)
            .writeU32(this.apk.client_ver) // app client ver
            .writeU64(this.uin)
            .write32((Date.now() / 1000) & 0xffffffff)
    ...
    0x112: function (uin) {
        return new writer_1.default().writeTlv(String(uin || this.uin));
    },
    0x116: function (appidList = [1600000226]) {
        const body = new writer_1.default()
            .writeU8(0)
            .writeU32(this.apk.bitmap)
            .writeU32(this.apk.sub_sig_map) // sub sigmap
            .writeU8(appidList.length); // size of app id list
    ...
    0x142: function () {
        return new writer_1.default().writeU16(0).writeTlv(this.apk.id.slice(0, 32));
    },
    0x143: function () {
    ...
    0x144: function (info) {
        ...
        if (info) {
            body.writeBytes(packTlv.call(this, 0x148, info));
    ...
    0x147: function () {
        return new writer_1.default().writeU32(this.apk.appid).writeTlv(this.apk.ver).writeTlv(this.apk.sign.length ? this.apk.sign : this.apk.id.slice(0, 32));
    },
    0x148: function (info) {
        return new writer_1.default()
            .writeTlv(info.id.slice(0, 32))
            .writeU32(info.appid)
            .writeU32(info.appid)
            .writeU32(info.appid === 16 ? 16 : info.subid)
            .writeTlv(info.ver)
            .writeTlv(info.sign.length ? info.sign : info.id.slice(0, 32));
    },
    0x153: function (isRoot = 0) {
        return new writer_1.default().writeU16(isRoot);
    },
    0x154: function () {
    ...
    0x177: function () {
        return new writer_1.default().writeU8(0x01).writeU32(this.apk.buildtime).writeTlv(this.apk.sdkver);
    },
    0x17a: function () {
    ...
    0x523: function () {
        return new writer_1.default().writeTlv(Buffer.from([0x1, 0x0]));
    },
    0x52d: function () {
        const d = this.device;
        const buf = pb.encode({
    ...
    0x542: function (data) {
        return new writer_1.default().writeBytes(data ||
            pb.encode({
                9: {
                    12: 1,
                },
    ...
    0x548: function () {
        ...
        const t548 = constants_1.calcPoW.call(this, t546);
