...
import { md5, BUF0, calcPoW, BUF4 } from "./constants.mjs";
import { sign } from "../internal/enctyption.mjs";
...
const map = {
    ...
    0x106: function (md5pass, nt = false) {
        if (!md5pass?.length)
            return new Writer().writeBytes(this.sig.a1);
    ...
    0x112: function (uin) {
        return new Writer().writeTlv(String(uin || this.uin));
    },
    ...
    0x147: function () {
        return new Writer().writeU32(this.apk.appid).writeTlv(this.apk.ver).writeTlv(this.apk.sign.length ? this.apk.sign : this.apk.id.slice(0, 32));
    },
    0x148: function (info) {
        return new Writer()
            .writeTlv(info.id.slice(0, 32))
    ...
    0x153: function (isRoot = 0) {
        return new Writer().writeU16(isRoot);
    },
    0x154: function () {
    ...
    0x542: function (data) {
        return new Writer().writeBytes(data ||
            pb.encode({
                9: {
                    12: 1,
                },
    ...
    0x544: function (v, subCmd, signData = BUF0) {
        if (v == -1) {
    ...
    0x548: function () {
        ...
        const t548 = calcPoW.call(this, t546);
