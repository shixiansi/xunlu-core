    ...
    toJSON(convertBigInt = false) {
        const toJSON = (buf) => {
            ...
            if (compress)
                return compress;
            const result = {};
            const reader = new minimal_js_1.default.Reader(buf);
...
function _encode(writer, tag, value) {
    ...
    if (typeof value === "number") {
        ...
        const tmp = new minimal_js_1.default.util.Long();
        tmp.unsigned = false;
...
function encode(obj) {
    ...
    const writer = new minimal_js_1.default.Writer();
...
function decode(encoded) {
    ...
    const reader = new minimal_js_1.default.Reader(encoded);
    while (reader.pos < reader.len) {
        const k = reader.uint32();
        const tag = k >> 3, type = k & 0b111;
        let value, decoded;
