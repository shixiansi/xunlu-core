...
import { formatTime, md5, randomString } from "./constants.mjs";
import axios from "axios";
import { aesDecrypt, aesEncrypt, encryptPKCS1 } from "./algo/index.mjs";
import fs from "fs";
...
export function generateShortDevice() {
    ...
    return {
        ...
        ip_address: `192.168.${parseInt(randstr(2, true))}.${parseInt(randstr(2, true))}`,
...
/** 生成完整设备信息 */
export function generateFullDevice(apk, d) {
    if (!d)
...
-----END PUBLIC KEY-----`;
    ...
    async getQIMEI() {
        ...
        try {
            const { data } = await axios.post("https://snowflake.qq.com/ola/android", {
                key,
                params,
                time,
                nonce,
    ...
    genRandomPayloadByDevice() {
        ...
        const dateFormat = (fmt, time = Date.now()) => formatTime(time, fmt);
...
(function (Platform) {
    ...
    /** 自定义 */
    Platform[Platform["Custom"] = 7] = "Custom";
})(Platform || (Platform = {}));
export var NTLoginPlatform;
(function (NTLoginPlatform) {
    NTLoginPlatform[NTLoginPlatform["Unknown"] = 0] = "Unknown";
    NTLoginPlatform[NTLoginPlatform["iPhone"] = 1] = "iPhone";
    NTLoginPlatform[NTLoginPlatform["Android"] = 2] = "Android";
    NTLoginPlatform[NTLoginPlatform["Symbian"] = 3] = "Symbian";
    NTLoginPlatform[NTLoginPlatform["Windows"] = 4] = "Windows";
    NTLoginPlatform[NTLoginPlatform["Mac"] = 5] = "Mac";
    NTLoginPlatform[NTLoginPlatform["iPad"] = 6] = "iPad";
    NTLoginPlatform[NTLoginPlatform["Linux"] = 7] = "Linux";
    NTLoginPlatform[NTLoginPlatform["Harmony"] = 8] = "Harmony";
})(NTLoginPlatform || (NTLoginPlatform = {}));
;
var MainSigMaps;
(function (MainSigMaps) {
    MainSigMaps[MainSigMaps["WLOGIN_A5"] = 2] = "WLOGIN_A5";
    MainSigMaps[MainSigMaps["WLOGIN_RESERVED"] = 16] = "WLOGIN_RESERVED";
    MainSigMaps[MainSigMaps["WLOGIN_STWEB"] = 32] = "WLOGIN_STWEB";
    MainSigMaps[MainSigMaps["WLOGIN_A2"] = 64] = "WLOGIN_A2";
    MainSigMaps[MainSigMaps["WLOGIN_ST"] = 128] = "WLOGIN_ST";
    MainSigMaps[MainSigMaps["WLOGIN_LSKEY"] = 512] = "WLOGIN_LSKEY";
    MainSigMaps[MainSigMaps["WLOGIN_SKEY"] = 4096] = "WLOGIN_SKEY";
    MainSigMaps[MainSigMaps["WLOGIN_SIG64"] = 8192] = "WLOGIN_SIG64";
    MainSigMaps[MainSigMaps["WLOGIN_OPENKEY"] = 16384] = "WLOGIN_OPENKEY";
    MainSigMaps[MainSigMaps["WLOGIN_TOKEN"] = 32768] = "WLOGIN_TOKEN";
    MainSigMaps[MainSigMaps["WLOGIN_VKEY"] = 131072] = "WLOGIN_VKEY";
    MainSigMaps[MainSigMaps["WLOGIN_D2"] = 262144] = "WLOGIN_D2";
    MainSigMaps[MainSigMaps["WLOGIN_SID"] = 524288] = "WLOGIN_SID";
    MainSigMaps[MainSigMaps["WLOGIN_PSKEY"] = 1048576] = "WLOGIN_PSKEY";
    MainSigMaps[MainSigMaps["WLOGIN_AQSIG"] = 2097152] = "WLOGIN_AQSIG";
    MainSigMaps[MainSigMaps["WLOGIN_LHSIG"] = 4194304] = "WLOGIN_LHSIG";
    MainSigMaps[MainSigMaps["WLOGIN_PAYTOKEN"] = 8388608] = "WLOGIN_PAYTOKEN";
    MainSigMaps[MainSigMaps["WLOGIN_PF"] = 16777216] = "WLOGIN_PF";
    MainSigMaps[MainSigMaps["WLOGIN_DA2"] = 33554432] = "WLOGIN_DA2";
    MainSigMaps[MainSigMaps["WLOGIN_QRPUSH"] = 67108864] = "WLOGIN_QRPUSH";
    MainSigMaps[MainSigMaps["WLOGIN_PT4Token"] = 134217728] = "WLOGIN_PT4Token";
})(MainSigMaps || (MainSigMaps = {}));
;
const mobile = [
    // 每个版本不同的信息
    {
...
function readCustomApkInfoList() {
    const defaultApkInfo = [mobile[0]].map((info) => {
        info = { ...info };
        delete info.apad_subid;
        info.sign = info.sign.toString("hex");
        return info;
    })[0];
    const apkInfoListPath = /^\d{5,10}|undefined$/.test(path.basename(this.config.data_dir))
        ? path.resolve(this.config.data_dir, "../custom-apks.json")
        : path.join(this.config.data_dir, "custom-apks.json");
    try {
        let apkInfoList = [defaultApkInfo];
        if (!fs.existsSync(apkInfoListPath)) {
            if (!fs.existsSync(path.dirname(apkInfoListPath)))
                fs.mkdirSync(path.dirname(apkInfoListPath), { recursive: true });
            fs.writeFileSync(apkInfoListPath, JSON.stringify([defaultApkInfo], null, 1));
            setTimeout(() => {
                this.emit("internal.verbose", "创建了自定义版本信息文件：" + apkInfoListPath, VerboseLevel.Mark);
            }, 500);
        }
        else {
            apkInfoList = JSON.parse(fs.readFileSync(apkInfoListPath).toString());
        }
        return apkInfoList.map(info => {
            info = { ...defaultApkInfo, nt: false, qr_appid: 0, fekit_ver: undefined, ...info };
            info.sign = Buffer.from(info.sign, "hex");
            return info;
        });
    }
    catch (e) {
...
export function getApkInfoList(p) {
    if (p === Platform.Custom)
        apklist[p] = readCustomApkInfoList.call(this);
    const apkItem = apklist[p];
    let apks;
    if (Array.isArray(apkItem)) {
        apks = apkItem;
    }
    else if (apkItem) {
        apks = [apkItem];
