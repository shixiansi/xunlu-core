...
function _encrypt(x, y, k0, k1, k2, k3) {
    for (let i = 0; i < 16; ++i) {
        let aa = ((_toUInt32(((y << 4) >>> 0) + k0) ^ _toUInt32(y + deltas[i])) >>> 0) ^
            _toUInt32(~~(y / 32) + k1);
        aa >>>= 0;
        x = _toUInt32(x + aa);
        let bb = ((_toUInt32(((x << 4) >>> 0) + k2) ^ _toUInt32(x + deltas[i])) >>> 0) ^
            _toUInt32(~~(x / 32) + k3);
        bb >>>= 0;
        y = _toUInt32(y + bb);
    }
...
function _decrypt(x, y, k0, k1, k2, k3) {
    for (let i = 15; i >= 0; --i) {
        let aa = ((_toUInt32(((x << 4) >>> 0) + k2) ^ _toUInt32(x + deltas[i])) >>> 0) ^
            _toUInt32(~~(x / 32) + k3);
        y = (y - aa) >>> 0;
        let bb = ((_toUInt32(((y << 4) >>> 0) + k0) ^ _toUInt32(y + deltas[i])) >>> 0) ^
            _toUInt32(~~(y / 32) + k1);
        x = (x - bb) >>> 0;
    }
    return [x, y];
