...
import { NOOP, timestamp, OnlineStatus, uin2code } from "../common.mjs";
import { PrivateMessage, GroupMessage, DiscussMessage, genDmMessageId, genGroupMessageId, } from "../message/index.mjs";
import { Proto } from "../core/protobuf/index.mjs";
import { uid2uin, uid2uins } from "./internal.mjs";
import { pbGetMsg } from "./pbgetmsg.mjs";
import { getFrdSysMsg, getGrpSysMsg } from "./sysmsg.mjs";
/** OnlinePush回执 */
...
export function emitGroupNoticeEvent(c, gid, e) {
    if (!e)
        return;
...
export function onlinePushListener(payload, seq) {
...
export function onlinePushTransListener(payload, seq) {
...
export function ntMsgListener(payload, seq) {
    const proto = pb.decode(payload);
    if (proto[4])
    ...
    switch (type) {
        ...
        case 44:
        case 85: {
            const gid = uin2code(proto[1][1][1]);
        ...
        default:
            if (["trace", "debug"].includes(this.config.log_level))
                this.logger.debug(`不支持的消息类型：${type},${sub_type},${payload?.length ? `${payload.toString("hex")},${pb.decode(payload).toJSONString(undefined, 1)}` : payload}`);
    }
}
    ...
    try {
        switch (type) {
            case 33: {
                ...
                const user_id = await uid2uin.call(this, user_uid, gid);
                let nickname = "";
                ...
                if (user_id === this.uin) {
                    ...
                    this.config.cache_group_member &&
                        (await g.pickMember(user_id).renew().catch(NOOP));
            ...
            case 34: {
                ...
                const [operator_id, user_id] = await uid2uins.call(this, [operator_uid, user_uid], gid);
            ...
            default:
                if (["trace", "debug"].includes(this.config.log_level))
                    this.logger.debug(`不支持的群事件：${type},${proto.toHex()},${proto instanceof Proto ? `${proto.toJSONString(undefined, 1)}` : proto}`);
        }
    }
    catch (err) {
        this.logger.error(err);
        this.logger.error(`事件处理异常：${type},${proto instanceof Proto ? proto.toHex() : proto}`);
    }
    if (event)
        ...
        switch (sub_type) {
            case 0x8: {
                ...
                this.em("sync.read.private", {
                    user_id: await uid2uin.call(this, String(data[1])),
                    timestamp: data[2],
                });
                break;
            }
            ...
            case 0x8b: {
                ...
                const uins = await uid2uins.call(this, [operator_uid, user_uid]);
            ...
            case 0xd4: {
                this.logger.debug(`[push528]${sub_type},${proto instanceof Proto ? proto.toHex() : proto}`);
                const gid = proto[2];
                this.pickGroup(gid).renew().catch(NOOP);
                break;
            }
            ...
            default:
                if (["trace", "debug"].includes(this.config.log_level))
                    this.logger.debug(`[push528]不支持的事件：${sub_type},${proto instanceof Proto ? `${proto.toHex()},${proto.toJSONString(undefined, 1)}` : proto}`);
        }
    }
    catch (err) {
        this.logger.error(err);
        this.logger.error(`[push528]事件处理异常：${sub_type},${proto instanceof Proto ? proto.toHex() : proto}`);
    }
    if (event)
...
async function ntSub0x27(uin, proto) {
    ...
    try {
        switch (type) {
            case 0: {
                const data = proto[1];
                this.classes.set(await uid2uin.call(this, String(data[3][1])), String(data[3][3]));
                break;
            }
            case 1: {
                const data = proto[1];
                this.classes.delete(await uid2uin.call(this, String(data[4][1])));
                break;
            }
            case 2: {
                const data = proto[1];
                this.classes.set(await uid2uin.call(this, String(data[5][1])), String(data[5][2]));
                break;
            }
            case 4: {
                const data = proto[1];
                const arr = Array.isArray(data[7][1]) ? data[7][1] : [data[7][1]];
                const uins = await uid2uins.call(this, arr.map(val => String(val[1])));
            ...
            case 20: {
                ...
                const user_uin = await uid2uin.call(this, user_uid);
            ...
            case 40: {
                ...
                const user_id = await uid2uin.call(this, user_uid, o[1] ? o[4] : 0);
            ...
            default:
                if (["trace", "debug"].includes(this.config.log_level))
                    this.logger.debug(`[sub0x27]不支持的事件：${type},${proto instanceof Proto ? `${proto.toHex()},${proto.toJSONString(undefined, 1)}` : proto}`);
        }
    }
    catch (err) {
        this.logger.error(err);
        this.logger.error(`[sub0x27]事件处理异常：${type},${proto instanceof Proto ? proto.toHex() : proto}`);
    }
    return event;
}
async function ntPush732(gid, sub_type, proto) {
    if (!(proto instanceof Proto && proto.checkTag(2)))
        return;
    let buf = proto[2].toBuffer(), event;
    try {
        proto = pb.decode(buf) || pb.decode(buf.subarray(7));
    }
    catch {
        proto = pb.decode(buf.subarray(7));
    }
    try {
        switch (sub_type) {
            case 0x1: {
            ...
            case 0x11: {
                ...
                const uins = await uid2uins.call(this, [operator_uid, user_uid], gid);
            ...
            case 0x0c: {
                if (!(proto instanceof Proto && proto.checkTag(4, 5)))
                    break;
            ...
            default:
                if (["trace", "debug"].includes(this.config.log_level))
                    this.logger.debug(`[push732]不支持的事件：${sub_type},${proto instanceof Proto ? `${proto.toHex()},${proto.toJSONString(undefined, 1)}` : proto}`);
        }
    }
    catch (err) {
        this.logger.error(err);
        this.logger.error(`[push732]事件处理异常：${sub_type},${proto instanceof Proto ? proto.toHex() : proto}`);
    }
    if (event)
        emitGroupNoticeEvent(this, gid, event);
}
...
function privateMsgListener(payload, seq, nt = false) {
    ...
    const msg = new PrivateMessage(this, proto, this.uin, nt);
    if (msg.raw_message) {
...
export function dmMsgSyncListener(payload, seq) {
    const proto = pb.decode(payload);
    handleOnlinePush.call(this, proto[2], seq);
    const msg = new PrivateMessage(this, proto[1], this.uin);
    msg.sender.nickname = this.nickname;
    this.em("sync.message", msg);
}
const fragmap = new Map();
export function groupMsgListener(payload, seq, nt = false) {
    this.stat.recv_msg_cnt++;
    if (!this._sync_cookie && !this.useQQNT)
        return;
    let msg = new GroupMessage(this, pb.decode(payload)[1], nt);
...
export function discussMsgListener(payload, seq) {
    this.statistics.recv_msg_cnt++;
    const proto = pb.decode(payload);
    handleOnlinePush.call(this, proto[2], seq);
    if (!this._sync_cookie && !this.useQQNT)
        return;
    const msg = new DiscussMessage(this, proto[1]);
    if (msg.user_id === this.uin && this.config.ignore_self)
        return;
    if (msg.raw_message) {
...
export function syncPushListener(payload) {
    //console.log('syncPushListener:', pb.decode(payload).toJSON())
}
export async function pushNotify(type) {
    switch (type) {
        case 33: //群员入群
        case 38: //建群
        case 85: //群申请被同意
