...
import { jce, NTLoginPlatform, QrcodeResult } from "../core/index.mjs";
import { BUF0, NOOP, OnlineStatus } from "../common.mjs";
import { pbGetMsg, pushReadedListener } from "./pbgetmsg.mjs";
import { dmMsgSyncListener, groupMsgListener, discussMsgListener, onlinePushListener, onlinePushTransListener, ntMsgListener, syncPushListener, pushNotify } from "./onlinepush.mjs";
import { guildMsgListener } from "./guild.mjs";
async function pushNotifyListener(payload) {
    ...
    pushNotify.call(this, nested[5]);
}
const events = {
    "trpc.msg.olpush.OlPushService.MsgPush": ntMsgListener,
...
async function onlineListener(token, nickname) {
    ...
    if (!this.status) {
        this.getOnlineStatus().then(status => {
            this.status = status || OnlineStatus.Online;
...
function authListener(info) {
    ...
    const device = {
        ...
        platform: String(NTLoginPlatform[this.apk.login_platform]),
...
export function bindInternalListeners() {
