import { pb, jce, Platform } from "../core/index.mjs";
import { drop } from "../errors.mjs";
import { timestamp, uuid, NOOP, BUF0, int32ip2str, calcBkn } from "../common.mjs";
...
export async function getStatusInfo(uin, usejce) {
    try {
        if (!usejce) {
            ...
            return {
                ...
                customOnlineStatusDesc: pb.decode(proto[9]?.encoded || BUF0),
                extOnlineStatus: proto[10],
                batteryStatus: proto[11],
                musicInfo: pb.decode(proto[12]?.encoded || BUF0),
                poiInfo: pb.decode(proto[13]?.encoded || BUF0),
                extOnlineBusinessInfo: pb.decode(proto[14]?.encoded || BUF0),
                extInfo: pb.decode(proto[15]?.encoded || BUF0),
                unknown: proto[16],
                onlineStatus: proto[17],
            };
        }
        else {
            const GetOnlineInfoReq = jce.encodeStruct([0, uin || this.uin, null, 41, 1]);
            const body = jce.encodeWrapper({ GetOnlineInfoReq }, "mqq.IMService.FriendListServiceServantObj", "GetOnlineInfoReq");
            const payload = await this.sendUni("friendlist.GetOnlineInfoReq", body);
            const data = jce.decodeWrapper(payload);
            return {
                result: data[0],
                errorCode: data[1],
                status: data[2],
                ...
                customOnlineStatusDesc: pb.decode(Buffer.from(data[10] || BUF0)),
                extOnlineStatus: data[11],
                batteryStatus: data[12],
                musicInfo: pb.decode(data[13] || BUF0),
                poiInfo: pb.decode(data[14] || BUF0),
                extOnlineBusinessInfo: pb.decode(data[15] || BUF0),
                extInfo: pb.decode(data[16] || BUF0),
            };
        }
    }
    catch { }
...
export async function setStatus(status) {
    if (!status || [Platform.Watch].includes(this.config.platform))
        return true;
    const d = this.device;
 ...
 */
export async function setSign(sign) {
 ...
 * @param sign {string} 签名
 */
export async function setPersonalSign(sign) {
 ...
 */
export async function getUserProfile(uin_uid, idsParse) {
...
export async function uid2uins(uids, group_id) {
...
export async function uid2uin(uid, group_id) {
    const uins = await uid2uins.call(this, [uid], group_id);
    return uins[0];
}
export async function uin2uids(uins, group_id) {
    const query_uins = [];
    for (let uin of uins) {
...
export async function uin2uid(uin, group_id) {
    const uids = await uin2uids.call(this, [uin], group_id);
    return uids[0];
}
export async function refreshBigDataSession() {
    const body = pb.encode({
        1281: {
            1: this.uin,
            2: 0,
    ...
    const decoded = pb.decode(payload)[1281];
    try {
...
export async function setAvatar(img) {
    await img.task;
    const body = pb.encode({
        1281: {
            1: this.uin,
...
export async function getStamp(no_cache = false) {
...
export async function delStamp(id) {
    const body = pb.encode({
        1: {
            1: 109,
            2: "7.1.2",
...
export function loadGPL() {
    this.sendUni("trpc.group_pro.synclogic.SyncLogic.SyncFirstView", pb.encode({ 1: 0, 2: 0, 3: 0 }))
        .then(payload => {
...
export async function imageOcr(img) {
    await img.task;
    ...
    if (rsp[3] !== 0)
        drop(rsp[3], rsp[5]);
    if (rsp[4]?.[1])
        drop(rsp[4][1], rsp[4][3] || rsp[4][2]);
    return new OcrResult(rsp[4][10]);
}
export async function refreshNTPicRkey(force = false) {
    const types = [10, 20, 34];
    if (!force && !types.filter(type => (this.sig.rkey_info[type]?.expire_time ?? 0) - timestamp() < 600).length) {
...
export async function getPSkey(domains) {
