...
import { uin2code, NOOP, timestamp } from "../common.mjs";
import { PrivateMessage } from "../message/index.mjs";
import { emitGroupNoticeEvent } from "./onlinepush.mjs";
/** @param seed 填一个常数 */
export function buildSyncCookie(seed) {
...
export async function pbGetMsg() {
    if (this.useQQNT)
        return;
...
async function handleSyncMsg(proto) {
    ...
    //私聊消息
        ...
        const msg = new PrivateMessage(this, proto, this.uin);
        if (msg.raw_message) {
...
export function pushReadedListener(payload) {
    if (this.useQQNT)
        return;
