...
/** highway上传数据 (只能上传流) */
async function highwayUpload(readable, obj, ip, port) {
    port = port || this.sig.bigdata.port;
    if (!port)
        await internal_1.refreshBigDataSession.call(this);
    port = port || this.sig.bigdata.port;
    ...
    return new Promise((resolve, reject) => {
        ...
        const createSocket = (ip, port) => {
            ...
            socket.on("close", (had_error) => {
                ...
                if (had_error) {
                    if (ip != (0, common_1.int32ip2str)(this.sig.bigdata.ip) && this.sig.bigdata.port) {
                        ...
                        networkErrorCount++;
                        internal_1.refreshBigDataSession.call(this).finally(() => {
                            createSocket((0, common_1.int32ip2str)(this.sig.bigdata.ip), this.sig.bigdata.port);
                        });
                        return;
                    }
...
async function highwayHttpUpload(readable, obj, ip, port) {
    port = port || this.sig.bigdata.port;
    if (!port)
        await internal_1.refreshBigDataSession.call(this);
    port = port || this.sig.bigdata.port;
