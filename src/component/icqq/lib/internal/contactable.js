...
exports.checkImgUrl = checkImgUrl;
exports.downloadFileToTmpDir = downloadFileToTmpDir;
exports.saveFileToTmpDir = saveFileToTmpDir;
exports.createReadable = createReadable;
exports.concatStreams = concatStreams;
exports.read7Bytes = read7Bytes;
...
class Contactable {
    ...
    async _ntMediaUp(elem) {
        ...
        switch (true) {
            case elem instanceof message_1.Image:
            case elem?.type === "image": {
                type = 1;
                cmd = `OidbSvcTrpcTcp.0x11c${this.dm ? "5" : "4"}_100`;
                const img = elem instanceof message_1.Image ? elem : new message_1.Image(this.c, elem);
                await img.task;
                extBizInfo[type] = {
                    1: img.asface ? 1 : 0,
                    2: img.summary,
            ...
            case elem instanceof message_1.Video:
            case elem?.type === "video":
            case elem?.type === "bubble": {
                type = (elem instanceof message_1.Video ? elem.elem_type : elem) === "bubble" ? 5 : 2;
                cmd = `OidbSvcTrpcTcp.0x11e${this.dm ? "9" : "a"}_100`;
                const video = elem instanceof message_1.Video ? elem : new message_1.Video(this.c, elem);
                await video.task;
            ...
            case elem instanceof message_1.Ptt:
            case elem?.type === "record": {
                type = 3;
                cmd = `OidbSvcTrpcTcp.0x126${this.dm ? "d" : "e"}_100`;
                const ptt = elem instanceof message_1.Ptt ? elem : new message_1.Ptt(this.c, elem);
                await ptt.task;
                extBizInfo[type] = {};
                fileInfos.push({
                    1: {
        ...
        if (this.dm)
            sceneInfo[201] = {
                1: 2,
                2: await internal_1.uin2uid.call(this.c, this.target, this.gid)
            };
        if (this.dm && this.gid)
            sceneInfo[201][3] = {
                3: {
        ...
        const proto = {
            ...
            2: {
                ...
                2: 1,
                3: 0,
                4: (0, crypto_1.randomBytes)(4).readUInt32BE(),
                5: this.dm ? 1 : 2,
                6: extBizInfo,
                7: 0,
                8: 0,
    ...
    async uploadImages(_imgs) {
        ...
        for (const img of _imgs) {
            ...
            else if (img instanceof message_1.Image && img.deleteTmpFile) {
                img.deleteTmpFile();
            }
        }
        ...
        for (let i = 0; i < imgs.length; i++) {
            if (!(imgs[i] instanceof message_1.Image)) {
                imgs[i] = new message_1.Image(this.c, imgs[i], this.dm, path_1.default.join(this.c.dir, "../image"));
            }
            const img = imgs[i];
            try {
    ...
    async _uploadImage(img) {
        ...
        if (img.uploadInfo.retcode !== 0) {
            img.deleteTmpFile();
            img.deleteCacheFile();
            (0, errors_1.drop)(img.uploadInfo.retcode, img.uploadInfo.message);
        }
    ...
    async share(content, config) {
        const request = (0, message_1.buildShare)((this.gid || this.uin), this.dm ? 0 : 1, content, config);
    ...
    async _uploadFile(elem, callback) {
        ...
        if (typeof elem.file === "string" && elem.file.startsWith("fid:")) {
            ...
            if (!fid.includes("_")) {
                if (!this.gid)
                    (0, errors_1.drop)(errors_1.ErrorCode.GroupFileNotExists);
        ...
        const file = new message_1.File(this.c, elem, this.dm);
        await file.task;
        if (this.dm) {
            if (info) {
    ...
    async _preprocess(content, source) {
        try {
            ...
            const converter = new message_1.Converter(this.c, content, {
                dm: this.dm,
    ...
    async uploadVideo(elem) {
        if (typeof elem.file === "string" && elem.file.startsWith("protobuf://"))
            return elem;
        const video = new message_1.Video(this.c, elem, this.dm);
        await video.task;
        if (video.nt || video.elem_type === "bubble") {
            ...
            try {
                ...
                if (video.thumbInfo && resp[2]?.[10]) {
                    const resp2 = Buffer.from(core_1.pb.encode({
                        1: resp[1],
                        2: {
                            1: resp[2][10][2],
                            2: resp[2][10][3],
                    ...
                    video.thumbInfo.setUploadResp(core_1.pb.decode(resp2));
                }
            }
            catch { }
        }
            ...
            const ext = core_1.pb.encode({
                ...
                5: {
                    ...
                    3: video.thumbInfo.md5 || common_1.BUF0,
            ...
            const resp = core_1.pb.decode(payload)[3];
            try {
                video.setUploadResp(resp);
            }
            catch { }
        ...
        try {
            if (video.uploadInfo && video.readable) {
                if (video.uploadInfo.retcode !== 0) {
                    ...
                    (0, errors_1.drop)(video.uploadInfo.retcode, video.uploadInfo.message || "");
                }
                if (video.nt) {
                    if (!video.uploaded) {
                        await highway_1.highwayUpload.call(this.c, video.readable, {
                    ...
                    if (!video.thumbInfo.uploaded && video.thumbInfo.readable && video.thumbInfo.uploadInfo) {
                        await highway_1.highwayUpload.call(this.c, video.thumbInfo.readable, {
        ...
        return {
            type: elem.type,
            file: "protobuf://" + Buffer.from(core_1.pb.encode(video.proto)).toString("base64"),
            nt: video.nt
        };
    }
    /** 上传一个语音以备发送 */
    async uploadPtt(elem) {
        ...
        const ptt = new message_1.Ptt(this.c, elem, this.dm);
        await ptt.task;
        if (ptt.nt) {
            ...
            const resp = core_1.pb.decode(payload)[5];
            try {
                ptt.setUploadResp(resp);
            }
            catch { }
        ...
        if (ptt.uploadInfo && ptt.readable) {
            if (ptt.uploadInfo.retcode !== 0) {
                if (ptt.temp)
                    ptt.deleteTmpFile();
                (0, errors_1.drop)(ptt.uploadInfo.retcode, ptt.uploadInfo.message || "");
            }
            if (!ptt.uploaded) {
                await highway_1.highwayUpload.call(this.c, ptt.readable, {
        ...
        return {
            type: "record",
            file: "protobuf://" + Buffer.from(core_1.pb.encode(ptt.proto)).toString("base64"),
            nt: ptt.nt
        };
    }
    ...
    async _uploadMultiMsg(compressed) {
        ...
        await highway_1.highwayUpload.call(this.c, stream_1.Readable.from(Buffer.from(buf), { objectMode: false, highWaterMark: 1024 * 256 }), {
    ...
    async uploadLongMsg(content, source, isNT = this.c.useQQNT) {
        const node = this.buildMultiMsgNode({
            ...
            time: (0, common_1.timestamp)(),
            maker: content instanceof message_1.Converter ? content : await this._preprocess(content, source)
        }, isNT);
        const compressed = (0, zlib_1.gzipSync)(core_1.pb.encode(isNT ? { 2: { 1: "MultiMsg", 2: { 1: node } } } : { 1: node }));
        let resid;
        try {
            resid = isNT
            ...
            if (e?.retcode === core_1.SSOErrorCode.ProductNoRight)
    ...
    async makeForwardMsg(msglist, isNT = this.c.useQQNT) {
        return {
            type: "json",
            data: (0, message_1.createMultiMsgJson)(await this._makeForwardMsg(msglist, isNT))
        };
    }
    async _makeForwardMsg(msglist, isNT = this.c.useQQNT) {
        ...
        for (const fake of msglist) {
            ...
            if (!this.dm && !fake.nickname && ((0, common_1.timestamp)() - this.c.pickMember(this.target, fake.user_id).update_time) > 600)
                await this.c.pickMember(this.target, fake.user_id).renew().catch(common_1.NOOP);
            ...
            for (let elem of (Array.isArray(maker.content) ? maker.content : [maker.content])) {
                ...
                if (["xml", "json"].includes(elem.type)) {
                    const multimsg_elem = (0, message_1.parseMultiMsg)(elem);
                    if (!multimsg_elem)
                        continue;
                    elem = multimsg_elem;
                }
                ...
                if (resid && fileName) {
                    const resp = elem.compressed?.length ? (0, zlib_1.unzipSync)(Buffer.from(elem.compressed, "base64")) : (isNT
                        ? await this._newDownloadMultiMsg(String(resid), this.dm ? 1 : 2)
                        : await this._downloadMultiMsg(String(resid), this.dm ? 1 : 2));
                    let downMultiMsg = core_1.pb.decode(resp)[2];
            ...
            fake.seq = fake.seq ?? (0, crypto_1.randomBytes)(2).readUInt16BE();
            fake.rand = fake.rand ?? (0, crypto_1.randomBytes)(4).readUInt16BE();
            ...
            nodes.push(this.buildMultiMsgNode({
                ...
                time: fake.time || (0, common_1.timestamp)(),
                seq: fake.seq,
                rand: fake.rand,
                maker
            }, isNT));
        ...
        const compressed = (0, zlib_1.gzipSync)(core_1.pb.encode({
            2: MultiMsg,
        }));
        let resid;
        try {
        ...
        return {
            type: "multimsg",
            resid,
            filename: (0, common_1.uuid)().toUpperCase(),
            preview,
            title: "转发的聊天记录",
    ...
    async getForwardMsg(resid, fileName = "MultiMsg", isNT = this.c.useQQNT) {
        ...
        try {
            ...
            for (let proto of a) {
                try {
                    ret.push(new message_1.ForwardMessage(this.c, proto, isNT));
                }
                catch { }
            }
        }
     ...
     */
    async getNTPicRkey() {
        const rkeyInfos = await internal_1.refreshNTPicRkey.call(this.c);
        return {
...
async function downloadFileToTmpDir(url, headers) {
...
async function saveFileToTmpDir(file) {
