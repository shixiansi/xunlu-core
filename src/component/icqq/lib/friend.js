...
class User extends internal_1.Contactable {
    ...
    async getChatHistory(time = 0, cnt = 20) {
        ...
        try {
            if (this.c.useQQNT && this.user_uid?.length) {
                ...
                const payload = await this.c.sendUni("trpc.msg.register_proxy.RegisterProxy.SsoGetRoamMsg", core_1.pb.encode(body), 6, { message_type: 32 });
                ...
                for (const proto of obj[7]) {
                    try {
                        messages.push(new message_1.PrivateMessage(this.c, proto, this.c.uin, true));
                    }
                    catch { }
                }
                return messages;
        ...
        for (const proto of obj[6]) {
            try {
                messages.push(new message_1.PrivateMessage(this.c, proto, this.c.uin));
            }
            catch { }
        }
        return messages;
    ...
    async sendMsg(content, source) {
        ...
        try {
            ...
            if (err?.code === errors_1.ErrorCode.SignApiError)
                throw err;
