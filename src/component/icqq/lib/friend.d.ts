...
import { FriendInfo, MemberInfo } from "./entities";
...
export declare class User extends Contactable {
    ...
    protected constructor(c: Client, uin: number, uid: string);
    /** 返回作为好友的实例 */
    ...
    getProfile(idsParse?: Record<number, {
        key: string;
        parse: (val: any) => any;
    }>): Promise<Record<string, any>>;
    getStatusInfo(usejce?: boolean): Promise<import("./common").StatusInfo | null>;
    private _getLastSeq;
    /**
...
export declare class Friend extends User {
     ...
     * @returns 文件信息
     */
    uploadFile(file: string | Buffer | File, name?: string, callback?: (percentage: string) => void): Promise<Omit<FileElem, "type"> & Record<"url", string>>;
    /**
     * 发送离线文件
     * @param file `string`表示从该本地文件路径获取，`Buffer`表示直接发送这段内容
     ...
     * @returns 文件id(撤回时使用)
     */
    sendFile(file: string | Buffer | File, filename?: string, callback?: (percentage: string) => void): Promise<string>;
    /**
     * 撤回离线文件
     * @param fid 文件id
     */
