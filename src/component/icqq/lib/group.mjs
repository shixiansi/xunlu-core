...
import { ErrorCode, ErrorMessage, drop } from "./errors.mjs";
import { timestamp, code2uin, PB_CONTENT, NOOP, lock, hide } from "./common.mjs";
import { Contactable, uid2uin } from "./internal/index.mjs";
import { GroupMessage, Image, parseGroupMessageId, genGroupMessageId, } from "./message/index.mjs";
import { Gfs } from "./gfs.mjs";
...
export class Group extends Discuss {
    ...
    async renew() {
        ...
        let info = {
            ...
            owner_id: !proto[1]?.encoded
                ? proto[1]
                : await uid2uin.call(this.c, proto[1].toString(), this.gid),
            admin_flag: !!proto[50],
        ...
        while (true) {
            ...
            for (let v of Array.isArray(proto[2]) ? proto[2] : [proto[2]]) {
                let info = {
                    ...
                    shutup_time: v[102] > timestamp() ? v[102] : 0,
                    update_time: 0,
    ...
    async sendMsg(content, source, anony = false) {
        ...
        const rsp = pb.decode(payload);
        if (rsp[1] !== 0) {
            this.c.logger.error(`failed to send: [Group: ${this.gid}] ${rsp[2] || ErrorMessage[rsp[1]]}(${rsp[1]})`);
            drop(rsp[1], rsp[2]);
        }
        else if (rsp.checkTag(11, 12) && rsp[11] > 0) {
            message_id = genGroupMessageId(this.gid, this.c.uin, rsp[11], rand, rsp[3]);
        }
        if (!message_id)
    ...
    async _getLastSeq() {
        ...
        if (this.c.useQQNT) {
            ...
            proto = pb.decode(payload)[4][1][3][22];
        }
        return proto;
    }
    /**
    ...
    async getChatHistory(seq = 0, cnt = 20) {
        ...
        try {
            if (this.c.useQQNT) {
                ...
                const obj = pb.decode(payload)[3], messages = [];
                if (obj[1] > 0 || !obj[6])
                    return [];
                !Array.isArray(obj[6]) && (obj[6] = [obj[6]]);
                for (const proto of obj[6]) {
                    try {
                        messages.push(new GroupMessage(this.c, proto, true));
                    }
                    catch { }
                }
                return messages;
        ...
        const obj = pb.decode(payload), messages = [];
        if (obj[1] > 0 || !obj[6])
            return [];
        !Array.isArray(obj[6]) && (obj[6] = [obj[6]]);
        for (const proto of obj[6]) {
            try {
                messages.push(new GroupMessage(this.c, proto));
            }
            catch { }
        }
        return messages;
    ...
    async getShareJson() {
        ...
        return JSON.parse(String(pb.decode(payload)["5"]));
    }
    /** 设置群头像 */
    async setAvatar(file) {
        const img = new Image(this.c, { type: "image", file });
        await img.task;
