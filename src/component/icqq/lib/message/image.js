...
class Image {
    ...
    parseFileParam(file) {
        ...
        if (hash.length !== 16)
            throw new Error("bad file param: " + file);
        this.md5 = hash;
        this.sha1 = sha1.length ? Buffer.from(sha1, "hex") : common_1.BUF0;
        if (!this.sha1.length)
            this.nt = false;
    ...
    async fromReadable(readable, timeout) {
        try {
            ...
            const [dimensions, md5, sha1] = await Promise.all([
                ...
                (0, common_1.shaStream)(readable),
                (0, common_1.pipeline)(readable, fs_1.default.createWriteStream(this.tmpfile)),
            ]);
            this.setProperties(dimensions);
            this.md5 = md5;
            this.sha1 = sha1;
            this.sha1Stream = this.nt ? await core_1.constants.calculateSha1StreamBytes(fs_1.default.createReadStream(this.tmpfile, { highWaterMark: 1024 * 256 })) : [];
    ...
    fromWeb(url, cache, headers, timeout) {
        if (this.cachedir) {
            ...
            if (cache) {
                try {
                    this.parseFileParam(fs_1.default.readFileSync(this.cachefile, "utf8"));
        ...
        return new Promise(async (resolve) => {
            ...
            this.cachefile &&
                fs_1.default.writeFile(this.cachefile, buildImageFileParam(this.md5.toString("hex"), this.nt ? this.sha1.toString("hex") : "", this.size, this.width, this.height, this.type), common_1.NOOP);
            resolve();
        });
    }
    fromLocal(file) {
        try {
            ...
            return new Promise(async (resolve) => {
                ...
                const readable = fs_1.default.createReadStream(file, { highWaterMark: 1024 * 256 });
                const [dimensions, md5, sha1] = await Promise.all([
                    // @ts-ignore
                    (0, probe_image_size_1.default)(readable, true),
                    (0, common_1.md5Stream)(readable),
    ...
    setUploadResp(resp) {
        ...
        if (this.nt) {
            cmdid = this.dm ? internal_1.CmdID.NTDmImage : internal_1.CmdID.NTGroupImage;
            resp = resp[2];
            this.proto = resp[6];
            ...
            cmdid = this.dm ? internal_1.CmdID.DmImage : internal_1.CmdID.GroupImage;
            const j = this.dm ? 1 : 0;
            retcode = resp[2 + j];
