...
const file_1 = require("./message/file");
...
class Gfs {
     ...
     * @param send 是否发送文件消息到群内
     * @returns 上传的文件属性
     */
    async upload(file, pid = "/", name, callback, send = true) {
        if (!(file instanceof file_1.File)) {
            file = new file_1.File(this.c, { type: "file", file, name });
        }
        await file.task;
        const body = {
            1: {
                ...
                5: String(pid),
                6: file.name,
                7: "/storage/emulated/0/Pictures/files/s/" + file.name,
                8: file.size,
                9: file.sha1,
                11: file.md5,
                15: 1,
        ...
        checkRsp(rsp);
        file.fid = String(rsp[7]);
        file.busid = rsp[6];
        if (!rsp[10] && file.readable) {
            ...
            await internal_1.highwayUpload.call(this.c, file.readable, {
                cmdid: internal_1.CmdID.GroupFile,
                callback,
                md5: file.md5,
                size: file.size,
                ext,
        ...
        return await (send ? this._feed(String(rsp[7]), rsp[6]) : this._resolve(String(rsp[7])));
    }
    /**
     * 将文件转发到当前群
     ...
     */
    async forward(stat, pid = "/", name, send = true) {
        const body = {
            1: {
                1: this.gid,
                2: 3,
    ...
    /**
     * 将离线(私聊)文件转发到当前群
     * @param fid 私聊文件fid或fileInfo
     * @param name 转发后的文件名，默认不变
     * @param send 是否发送文件消息到群内
     * @returns 转发的文件在当前群的属性
     */
    async forwardOfflineFile(fid, name, send = true) {
        const info = fid?.fid ? fid : await this.c.pickFriend(this.c.uin).getFileInfo(fid);
        fid = info.fid;
        const body = core_1.pb.encode({
            1: 60100,
            2: 0,
        ...
        return await (send ? this._feed(String(rsp[30]), rsp[50]) : this._resolve(String(rsp[30])));
    }
    /**
     * 获取文件下载地址
     * @param fid 文件id
    ...
    async download(fid) {
        ...
        return {
            name: file.name,
            url: encodeURI(`http://${rsp[5] || rsp[4]}/ftn_handler/${rsp[6].toHex()}/?fname=${file.name}`),
            size: file.size,
            md5: file.md5,
