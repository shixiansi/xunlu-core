...
import { ApiRejection, BaseClient, generateShortDevice, pb, Platform, } from "./core/index.mjs";
import { calcBkn, hide, lock, md5, NOOP, OnlineStatus, timestamp } from "./common.mjs";
import { addClass, bindInternalListeners, delClass, delStamp, getUserProfile, getStamp, getSysMsg, imageOcr, loadBL, loadFL, loadGL, loadGPL, loadSL, parseFriendRequestFlag, parseGroupRequestFlag, renameClass, setAvatar, setPersonalSign, setStatus, getStatus, uid2uin, uid2uins, uin2uid, uin2uids, getStatusInfo, refreshNTPicRkey, getPSkey, getClientKey, } from "./internal/index.mjs";
import { Friend, User } from "./friend.mjs";
import { Discuss, Group } from "./group.mjs";
import { Member } from "./member.mjs";
import { Image, parseDmMessageId, parseGroupMessageId, } from "./message/index.mjs";
...
export class Client extends BaseClient {
    ...
    get client_key() {
        return this.sig.st_web.toString("hex");
    }
    get skey() {
        return this.sig.skey.toString();
    }
    /** csrf token */
    get bkn() {
        return calcBkn(this.sig.skey);
    }
    /** 数据统计 */
    get stat() {
    ...
    constructor(uin, conf) {
         ...
         */
        this.pickDiscuss = Discuss.as.bind(this);
        /**
        ...
        /** 相当于频道中的qq号 */
        this.tiny_id = "";
        this.g_tk = new Proxy(this.pskey, {
            get: (obj, domain) => {
                return obj[domain] ? calcBkn(obj[domain]) : 0;
            },
            set: () => {
                return false;
            },
        ...
        this.cookies = new Proxy(this.pskey, {
            get: (obj, domain) => {
                const cookie = [];
                const uin = String(this.uin).padStart(10, "0");
                cookie.push(`uin=o${uin}`, `skey=${this.sig.skey}`);
                if (obj[domain]) {
                    if (this.uid)
                        cookie.push(`p_uid=${this.uid}`);
                    cookie.push(`p_uin=o${uin}`, `p_skey=${obj[domain]}`);
                }
                return cookie.join("; ");
            },
            set: () => {
    ...
    async login(uin, password) {
        ...
        this.uin = uin || this.uin || 0;
        if (await this.switchQQVer()) {
            this.logger.mark(`[${this.uin}]获取到签名Api协议版本：${this.statistics.ver}`);
        }
        const apk_info = `${this.apk.display}_${this.apk.version}`;
        this.logger.mark(`[${this.uin}]使用协议：${apk_info}`);
        await this.updateCmdWhiteList();
        this.login_timer = null;
        try {
            if (!this.uin)
                throw new Error();
            const old_token_path = path.join(this.dir, this.uin + "_token");
            ...
            if (!fs.existsSync(token_path) && fs.existsSync(old_token_path)) {
                if (this.token_retry_num <= this.sig.token_retry_count)
                    throw new Error();
                const token = await fs.promises.readFile(old_token_path);
                return this.tokenLogin(token);
            }
            if (fs.existsSync(old_token_path))
                fs.unlinkSync(old_token_path);
            ...
            if (this.password_md5 && this.uin) {
                ...
                return this.passwordLogin(this.uin, this.password_md5);
            }
            else {
    ...
    getClientKey() {
        return getClientKey.call(this);
    }
    getPSkey(domains) {
        return getPSkey.call(this, domains);
    }
    refreshNTPicRkey(force = false) {
        return refreshNTPicRkey.call(this, force);
    }
    uid2uin(uid, group_id) {
        return uid2uin.call(this, uid, group_id);
    }
    uid2uins(uids, group_id) {
        return uid2uins.call(this, uids, group_id);
    }
    uin2uid(uin, group_id) {
        return uin2uid.call(this, uin, group_id);
    }
    uin2uids(uins, group_id) {
        return uin2uids.call(this, uins, group_id);
    }
    getStatusInfo(uin, usejce) {
        return getStatusInfo.call(this, uin, usejce);
    }
    /** 获取在线状态 */
    getOnlineStatus() {
     ...
     */
    async getProfile(uin_uid, idsParse) {
        return await getUserProfile.call(this, uin_uid, idsParse);
    }
    /** 设置头像 */
    async setAvatar(file) {
        return setAvatar.call(this, new Image(this, { type: "image", file }));
    }
    /** 获取漫游表情 */
    ...
    /** Ocr图片转文字 */
    imageOcr(file) {
        return imageOcr.call(this, new Image(this, { type: "image", file, nt: false }));
    }
    ...
    async getGroupShareJson(group_id) {
        return this.pickGroup(group_id).getShareJson();
    }
    /**
     * 获取子频道列表
    ...
    async getForumUrl(guild_id, channel_id, forum_id) {
        const payload = await this.sendOidbSvcTrpcTcp("OidbSvcTrpcTcp.0x10c3_1", pb.encode({
            1: BigInt(guild_id),
            2: Number(channel_id),
            3: {
                1: 2,
